...
function ListarPartidas() {
  ...
  const fetchPartidas = async () => {
    try {
      ...
      // Asegurarse de que filtroJugadores sea un número entero antes de enviarlo
      if (filtroJugadores) {
        const numJugadores = parseInt(filtroJugadores, 10);
  ...
  const partidasFiltradas = partidas.filter((partida) =>
    partida.name.toLowerCase().includes(filtroNombre.toLowerCase()) &&
    (filtroJugadores === "" || partida.current_players >= parseInt(filtroJugadores, 10)) &&
    partida.current_players < partida.max_players // Solo muestra partidas que no están llenas
  );
